#!/usr/bin/env python3
"""
Adam+Clamp vs HLCP+GIA on CIFAR-100, bs=10/20/40, KNOWN but MIXED (repeated) labels, no pool.
Labels are randomly sampled from [0, num_classes), allowing duplicates.
"""
...
OUTDIR = "expdata/gia_clamp_vs_hlcp_mixed"
os.makedirs(OUTDIR, exist_ok=True)

from _common import cosine_grad_loss, save_batch_img, total_variation
from cnn import BasicCNN, get_loaders, gia_reconstruct_batch
import cnn_data
from cnn_metrics import compute_psnr_batch, compute_ssim_batch


def adam_clamp_gia(model, target, true_grads, device, mean_t, std_t,
                   real_for_eval, steps=6000, lr=0.1, tv_weight=1e-4,
                   n_restarts=5):
    model.eval()
...
# Load full training set for sampling
random.seed(SEED)
np.random.seed(SEED)
...
# Build a pool of samples indexed by label
all_data = []
all_labels = []
...
for BS in [10, 20, 40]:
    print(f"\n{'#'*70}")
    print(f"# Batch Size = {BS}, KNOWN + MIXED (repeated) labels")
    print(f"{'#'*70}")

    # Randomly sample BS images (labels will naturally repeat for large BS)
    torch.manual_seed(SEED + BS)
    indices = torch.randperm(len(all_data))[:BS]
    data = all_data[indices].to(DEVICE)
    target = all_labels[indices].to(DEVICE)
    print(f"Labels: {target.cpu().tolist()}")
    unique_labels = len(target.unique())
    print(f"Unique labels: {unique_labels}/{BS}")
    ...
    # Adam+Clamp (known labels)
    print(f"\n--- Adam+Clamp known (bs={BS}, 5 restarts) ---")
    dummy_clamp = adam_clamp_gia(
        model, target, true_grads, DEVICE, mean_t, std_t,
        real_for_eval=real_unnorm, steps=6000, lr=0.1,
        tv_weight=1e-4, n_restarts=5)
    recon_clamp = unnorm(dummy_clamp)

    # HLCP+GIA (known labels)
    print(f"\n--- HLCP+GIA known (bs={BS}, 3 restarts) ---")
    best_feat = None
    ...
    print(f"\n{'='*60}")
    print(f"Results (bs={BS}, CIFAR-100, KNOWN+MIXED labels, no pool):")
