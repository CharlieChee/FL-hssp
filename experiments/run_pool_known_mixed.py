#!/usr/bin/env python3
"""Pool=True, KNOWN + MIXED labels, bs=10/20/40"""
...
OUTDIR = "expdata/pool_known_mixed"
...
for BS in [10, 20, 40]:
    print(f"\n{'#'*70}\n# BS={BS}, KNOWN+MIXED, Pool=True\n{'#'*70}")
    torch.manual_seed(SEED + BS)
    idx = torch.randperm(len(all_data))[:BS]
    data = all_data[idx].to(DEVICE); target = all_labels[idx].to(DEVICE)
    print(f"Labels: {target.cpu().tolist()}, Unique: {len(target.unique())}/{BS}")
    ...
    print(f"\n--- Adam+Clamp (bs={BS}) ---")
    dc = adam_clamp_gia(model, target, true_grads, DEVICE, mean_t, std_t, real_unnorm)
    rc = unnorm(dc)
    ...
    print(f"\n{'='*60}\nResults (bs={BS}, Pool=True, KNOWN+MIXED):")
