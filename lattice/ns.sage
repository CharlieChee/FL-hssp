...
def ns_original(H,MO): 
  ...
  #cntfount=len([True for NSi in NSo if sum(NSi)==2])
  #print(cntfount)
 # print(Y)
 # print(sum(Y))
...
def ns_hlcp(H,MO,B=1):
  ...
  return beta,tt2,nrafound,textra,ra,Y



def bit_guessing_modified(n, kappa, MO, x0, a, X, b, m):
    print("bit_guessing_modified")
    p = 3
    K = GF(p)
    t2 = cputime()
    xt23 = Matrix(K,
                  [(-x).list() + [x[i] * x[j] * ((i == j) and 1 or 2) for i in range(n) for j in range(i, n)] for x in
                   MO.T])
    ...
    if kappa != -1:
        ones = vector([1 for _ in range(m)])
        li = NS.rows()
        for NSi in NS:
            if ones - NSi not in li:
                li.append(ones - NSi)
    ...
    if kappa != -1:
        ...
        while True:
            ...
            i += 1
            if (i>=500):
                raise RuntimeError(f"Runtime error occurred: ns.sage bit_guessing_modified i>500 loop")
        print("Switching rounds", i, "time= %.1f" % cputime(ts))
        tsf = cputime(ts)
    ...
    # Assuming the recovery step as in Step2_BKZ
    B = 1  # Assuming B=1 for binary recovery, change as needed
    if B == 1:
        MB = recoverBinary(NS, kappa)
    else:
        MB = recoverBounded(NS, B)

    beta = n  # Assuming beta is n as there's no specific beta in bit_guessing

    return MB, beta,recovered
