...
def hssp_attack(H,alg='default'):    
 ...
 #    if kappa>0:
#      tei, tef, tsf,tt2=bit_guessing(H.n,H.kappa,MO,H.x0,H.a,H.X,H.b,H.m) 
 #     tttot=cputime(t)
 #     return  tt1,tt10,tt1O, tei, tef, tsf, tt2, tttot, nrafound, H
 #    else:
#      tei, tef, tt2=eigen(H.n,H.kappa,MO,H.x0,H.a,H.X,H.b,H.m) 
#      tttot=cputime(t) 
#     return  tt1
  ...
  if alg=='statistical':
    ...
    MO,tt1,tt10,tt1O= Step1(H.n,H.kappa,H.x0,H.a,H.X,H.b,H.m,BKZ=True)
    print ("\nStatistical Attack")     
    MOn, MO_lll = statistical_1(MO, H.n, H.m, H.x0, H.X, H.a, H.b, H.kappa)
    tica, tt2, nrafound, _nfound = statistical_2(
        MOn, MO_lll, H.n, H.m, H.x0, H.X, H.a, H.b, H.kappa
    )
    tttot=cputime(t)
...
def statHSSP(mi=70, ma=90, M=0,A='default',tablepr=True):
  L=[]
...
#change ma as the maximum number of n 
def statHSSPk(sigma,mi=70, ma=90, M=0,A='default',tablepr=True):
  L=[]
  for n in range(mi,ma,20):
    print
    kappa=int(sigma*n)
